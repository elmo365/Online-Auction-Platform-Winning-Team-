...
<?php
    ...
    if (isset($_POST['signup_btn'])) {
        ...
        if (count($errors) === 0) {
            ...
            $query = "INSERT INTO users SET username=?, firstname=?, surname=?, email=?, password=?, omang_passport=?, profile_picture=?, created_at=?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param('ssssssss', $username, $firstname, $surname, $email, $password, $omang, $profile_picture, $created_at);
            $result = $stmt->execute();
            if ($result) {
